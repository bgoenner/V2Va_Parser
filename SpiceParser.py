...
def Parse_Verilog(
    ...
    for soln in solnDF.groupby(['Solution']):
        ...
        # SPfile will be closed in this operation
        sim_time_out_lines = simulationTime(timeDF, SPfile, probeList, outputWords)
        SPfile = open(SP_outputFile_name_new, 'a')

        SPfile.write(sim_time_out_lines)
...
def simulationTime(timeDF, ofile, probe_list=None, outputWords=None):

    ofile_name = ofile.name
    ofile.close()

    ofile_temp_r = ofile.name+'.read'
    shutil.copyfile(ofile.name, ofile_temp_r)

    of_read = open(ofile_temp_r, 'r')

    ofile_temp_w = ofile.name+'.write'

    of_write = open(ofile_temp_w, 'w+')
    ...
    if probe_list is not None:
        if outputWords == None:
            raise ValueError('outputWords undef in simTime')
        
        VA_line_str += simulationProbes_tran(probe_list=probe_list, outputWords=outputWords, read_netlist=of_read, write_netlist=of_write)
    
        of_read.close()
        of_write.close()
        

        of_read2 = open(ofile_temp_w, 'r')
...
def simulationProbes_tran(probe_list, outputWords=None, parser='Xyce', read_netlist=None, write_netlist=None):

    pr_list = []

    read_name = read_netlist.name
    #print(probe_list)

    if probe_list['pressure'] is not None:
        print('\n\nAdding pressure probes...\n')
        for p in probe_list['pressure']:
            #if p.getProbeType() == 'pressure':
            pr_list.append('V('+p.getNode()+'_0)')

    if probe_list['flow'] is not None:
        print('\n\nAdding flow probes...\n')
        if read_netlist == None or write_netlist == None:
            raise ValueError('Cannot parse flow without netlist files')
        volt_probe   = {}
        node_replace = {}
        #r_buffer = io.StringIO(read_netlist.read())
        for line in read_netlist:
            for ind, p in enumerate(probe_list['flow']):
                #if p.getProbeType() == 'flow':
                # go through currnet netlist

                # replace nodes
                if ' '+p.getNode()+' ' in line:
                    line = line.replace(p.getNode()+'_0', p.getNode()+'_p_0')
                    
                    volt_probe['vp_'+str(ind)] = {
                        'node_i':p.getNode()+'_0',
                        'node_o':p.getNode()+'_p_0' 
                        }
            print(line)
            write_netlist.write(line)

        #node_replace = collections.OrderedDict(sorted(node_replace.items()))
        print(node_replace)

        #read_netlist.close()
        #read_netlist = open(read_name)
        #for ind, line in enumerate(read_netlist):
        #    print(line)
        #    if str(ind) in node_replace:
        #        line = line.replace(node_replace[ind]['node'], node_replace[ind]['replace']) 
        #    write_netlist.write(line)

        VA_volt_probe_lines = '\n\n*voltage probes\n'
        for vp in volt_probe:

            VA_volt_probe_lines += vp+' '+ \
                volt_probe[vp]['node_i']+' '+ \
                volt_probe[vp]['node_o']+' 0V\n'
            
            pr_list.append('I('+vp+')')
        write_netlist.write(VA_volt_probe_lines)
    
    if probe_list['concentration'] is not None:
        print('\n\nAdding concentration probes...\n')
        for p in probe_list['concentration']:
            #if p.getProbeType() == 'concentration':
            if p.getNode() in outputWords:
                pr_list.append('V('+p.getNode()+'_chc)')
            else:
                pr_list.append('V('+p.getNode()+'_0c)')

    VA_line_str =  '.print tran '
    VA_line_str += ' '.join(pr_list)
